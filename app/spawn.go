	...
	for i, b := range data {
		...
		if prefix {
			...
			case macroMarkfilePathWithComma:
				if !nonQuote {
					src = `'"` + strings.Join(g.Dir().MarkfilePaths(), `"', '"`) + `"'` // some filename with space need this. some.exe '"bla bla"'
				} else {
					src = `'` + strings.Join(g.Dir().MarkfilePaths(), `', '`) + `'` //windows needs single quote for everey path. powershell invoke-item does not need ".
				}
				src = ifElseSting(runtime.GOOS == "windows", strings.ReplaceAll(src, `\`, `/`), src)
